...
class SwdeToPostgis:
    ...
    pzgdic = {} #slownik w ktorym beda przechowywane punkty zalamania granic - identyfikator oraz x i y

#------------------------------------------------------------------------------------------------------#    
    def __init__(self, swde_file, tableListString, srid, pyproj4strFrom, pyproj4strTo, rodz_importu, pgserver, pgbase, pguser, pguserpswd,  txtcodec, id_zd):
        
        self.swde_file = swde_file
        self.tableListString = tableListString
        self.srid = srid
        self.pyproj4strFrom = pyproj4strFrom
        self.pyproj4strTo = pyproj4strTo
        self.rodz_importu = rodz_importu
        self.pgserver = pgserver
        self.pgbase = pgbase
        self.pguser = pguser
        self.pguserpswd = pguserpswd
        self.txtcodec = txtcodec
        self.id_zd = id_zd        
        
#------------------------------------------------------------------------------------------------------#    
    def pzg_struct(self,swde_file):
                ...
                try:
                    for line in f.readlines():
                        ...
                        if StringBetweenChar(line, ',', 0) == "NS" and StringBetweenChar(line, ',', 1) == "ZD":
                            self.id_jed_rej = StringBetweenChar(line, ',', 2)
                    ...
                    print "ilosc linii pliku:", str(self.ilosc_linii)
        
                    print "ilosc puntkow zalamania granicy", str(ilosc_pzg)
                    self.id_jed_rej = self.id_jed_rej.strip()
                    if self.id_jed_rej !="":
                        print "Znaleziony identyfikator jednostki rejestrowej: ",  self.id_jed_rej,  u" zostanie wykorzystany  w bazie do stworzenia kluczy obiektów. Jeśli chcesz użyć innego możesz go zastąpić wpisanym przez siebie tekstem"
                    else:
        ...
        print u"ilość linii pliku:", str(self.ilosc_linii)
...
#--------------------------------------------------------------------------------------------------------#
    def importuj_plik(self):
        ...
        self.pzg_struct(self.swde_file)
