-- Function: public.__g5sp_dze_arrins(character varying[])

-- DROP FUNCTION public.__g5sp_dze_arrins(character varying[]);

CREATE OR REPLACE FUNCTION public.__g5sp_dze_arrins(IN ins_arr character varying[])
  RETURNS TABLE(ins_g5id1 text, ins_g5npe text, udw_g5ud text, udw_g5rjdr text, jdr_g5ijr text, dze_g5id1 text, dze_g5idd text, dze_nr text, obr_nazwa text, jew_nazwa text, dze_idzd text, dze_geom geometry) AS
$BODY$
DECLARE
	idzd text;
    	query_char text;
...
BEGIN
FOR ins_char IN (SELECT unnest(ins_arr)) 
	LOOP
	query_char = '%' || ins_char || '%';
					...
					LOOP
						...
						for rec_dze in (Select g5dze.g5id1, g5dze.g5idd, g5dze.nr, g5dze.nrobr, g5dze.geom from public.g5dze
							...
							LOOP
								...
								dze_nr = rec_dze.nr; dze_nrobr = rec_dze.nrobr; dze_idzd = idzd;
